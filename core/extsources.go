...
// jacked from gospider for testing
...
func getWaybackURLs(domain string, noSubs bool) ([]wurl, error) {
	...
	res, err := http.Get(
		fmt.Sprintf("http://web.archive.org/cdx/search/cdx?url=%s%s/*&output=json&collapse=urlkey&fl=original", subsWildcard, domain),
		// &fl=original
	)
	if err != nil {
		return []wurl{}, err
	...
	for _, urls := range wrapper {
		...
		out = append(out, wurl{date: "", url: urls[0]})
	}
...
func getCommonCrawlURLs(domain string, noSubs bool) ([]wurl, error) {
	...
	//added code to fetch latest api URL from http://index.commoncrawl.org/collinfo.json
	res, err := http.Get("http://index.commoncrawl.org/collinfo.json")
	if err != nil {
		return []wurl{}, err
	}
	defer res.Body.Close()
	dabod, errb := ioutil.ReadAll(res.Body)
	if errb != nil {
		return []wurl{}, err
	}

	var curAPIlist CcApiList
	err = json.Unmarshal(dabod, &curAPIlist)

	latest := curAPIlist[0].CdxAPI

	// original function starts back up here
	res, err = http.Get(
		fmt.Sprintf("%s?url=%s%s/*&output=json", latest, subsWildcard, domain),
	)
	if err != nil {
		return []wurl{}, err
	}
...
func getVirusTotalURLs(domain string, noSubs bool) ([]wurl, error) {
	...
	if apiKey == "" {
		log.Warnf("You are not set VirusTotal API Key yet.")
		return out, nil
	}
